...
function validate(word) {
    const regex = /^[A-Za-z]+$/; 
    if (!regex.test(word)) {
        alert("Iltimos, faqat harflar kiriting. Raqam yoki maxsus belgilar kiritaolmaysiz.");
        return false;
    }
    return true;
}
...
searchBtn.addEventListener('click', () => {
    ...
    if (word && validate(word)) {  // Validate funksiyasini chaqirish
        ...
        resultDiv.innerHTML = '<p style="color:red;">Iltimos, so\'z kiriting!</p>';
    }
});
